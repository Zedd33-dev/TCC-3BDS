...
def reports(request):
    ...
    status_data = list(
        Animal.objects.values('status').annotate(total=Count('id')).order_by('status')
    )
    cost_by_category = list(
        CostEntry.objects.filter(date__gte=month_start, date__lte=today)
        .values('category')
        .annotate(total=Sum('amount_brl'))
        .order_by('-total')
    )
    top_sales = Sale.objects.select_related('animal').order_by('-price_brl')[:5]
    farm_inventory = list(
        Farm.objects.annotate(active_animals=Count('animals', filter=Q(animals__status=Animal.Status.ACTIVE)))
        .values('name', 'active_animals')
        .order_by('-active_animals', 'name')
    )
...
def animal_list(request):
    ...
    return render(request, 'animals/list.html', {
        ...
        'stats': {
            'total': animals.count(),
            'active': animals.filter(status=Animal.Status.ACTIVE).count(),
            'sold': animals.filter(status=Animal.Status.SOLD).count(),
            'female': animals.filter(sex=Animal.Sex.FEMALE).count(),
        },
    })
