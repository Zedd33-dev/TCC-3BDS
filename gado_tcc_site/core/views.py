...
def dashboard(request):
    ...
    upcoming_vaccinations = Vaccination.objects.select_related('animal').filter(
        next_due__isnull=False, next_due__gte=today, next_due__lte=next_30
    ).order_by('next_due')[:5]
    upcoming_treatments = Treatment.objects.select_related('animal').filter(
        next_due__isnull=False, next_due__gte=today, next_due__lte=next_30
    ).order_by('next_due')[:5]
...
# --- helpers ---
def _crud_list(request, template, qs, ctx=None, per_page=12):
    ctx = ctx or {}
...
def animal_list(request):
    ...
    page_obj = paginator.get_page(request.GET.get('page'))
    return render(request, 'animals/list.html', {'filter': f, 'items': page_obj.object_list, 'page_obj': page_obj, 'title': 'Animais'})
